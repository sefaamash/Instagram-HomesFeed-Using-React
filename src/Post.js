...
function Post({postId,user,caption,username,imageUrl}) {
    ...
    const postComment=(event)=>{
        ...
        db.collection('posts').doc(postId).collection('comments').add({
            text:comment,
            username: user.displayName,  //we r passing signed in user not the user who post that the thing to remmeber so we gave user inargument in which we map our user
    ...
    useEffect(() => {
        ...
        return()=>{
            unsubscribe();// we just call that unsubscribe value we can put all these directly in return func too no need of if above and gave inpu postid if postifd is there than it will work
    ...
    return (
        <div className='post'>
            ...
            <h3 classname='post_us'>{username}</h3>
            </div>
            ...
            <img className='post_image' src={imageUrl}  alt=""/>
              ...
              <form className='commentbox'>
                  ...
                  onChange={(e)=>setcomment(e.target.value)}/>
                   <button className='commentbtn' disabled={!comment} type='submit'  onClick={postComment} >Post</button>
